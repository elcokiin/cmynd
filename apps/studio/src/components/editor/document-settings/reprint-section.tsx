...
export function ReprintSection({ documentId }: ReprintSectionProps) {
  ...
  useEffect(() => {
    ...
    form.setFieldValue("type", document.type === "reprint" ? "reprint" : "own");
    form.setFieldValue(
      "originalAuthor",
      document.reprint?.originalAuthor ?? "",
    );
    form.setFieldValue("originalAuthorId", document.reprint?.originalAuthorId);
    form.setFieldValue("originalTitle", document.reprint?.originalTitle ?? "");
    form.setFieldValue(
      "originalDate",
                    ...
                    {(field) => (
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          field.handleChange(checked ? "reprint" : "own")
                        }
                      />
                    )}
                  </form.Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                <form.Field
                  ...
                  validators={{
                    onChange: ({ value }) => {
                      if (form.state.values.type !== "reprint")
                        return undefined;
                ...
                >
                  {(field) => (
                    <>
                      {isReprint ? (
                        <AuthorSearchCommand
                          initialAuthorId={state.values.originalAuthorId}
                          onSelect={(name, id: Id<"authors">) => {
              ...
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="reprintNotes" className="text-sm font-medium">
                  Why do you want to publish this on blog.elcokiin?
                </Label>
                <form.Field
                  name="notes"
                  validators={{
