...
export function AuthorSearchCommand({
	onSelect,
	initialAuthorId,
}: AuthorSearchCommandProps) {
	const [authorSearch, setAuthorSearch] = useState("");
	const [showCreateDialog, setShowCreateDialog] = useState(false);

	const reprintedAuthors = useQuery(api.authors.queries.listOriginalAuthors, {
		paginationOpts: { numItems: 100, cursor: null },
	});

	useEffect(() => {
		if (initialAuthorId && reprintedAuthors) {
			const author = reprintedAuthors.page.find(
				(a) => a._id === initialAuthorId,
			);
			if (author) {
				setAuthorSearch(author.name);
			}
		}
	}, [initialAuthorId, reprintedAuthors]);
	...
	const showEmpty =
		reprintedAuthors && displayedAuthors && displayedAuthors.length === 0;
	const showList = displayedAuthors && displayedAuthors.length > 0;

	const handleCreateSuccess = (name: string, id: Id<"authors">) => {
