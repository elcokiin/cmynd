...
import { ImageIcon, XIcon, PlusIcon, FileIcon, Trash2Icon, RotateCcwIcon } from "lucide-react";
...
export function ProjectImageManager({
  ...
  const handleRemoveSaved = useCallback(
    (index: number) => {
      ...
      if (image.storageId) {
        if (removedStorageIds.includes(image.storageId)) {
          onRemovedChange(
            removedStorageIds.filter((id) => id !== image.storageId),
          );
        } else {
          onRemovedChange([...removedStorageIds, image.storageId]);
        }
      } else {
    ...
    [images, onChange, removedStorageIds, onRemovedChange],
  );
        ...
        {images.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              {images.map((img, i) => {
                const isRemoved = img.storageId ? removedStorageIds.includes(img.storageId) : false;
                return (
                  <div
                    key={i}
                    className={cn(
                      "relative group rounded-lg border overflow-hidden",
                      isRemoved ? "border-destructive/40 bg-destructive/5" : "bg-muted/30",
                    )}
                  >
                    <div className={cn(isRemoved && "opacity-50 pointer-events-none")}>
                    ...
                    {isRemoved ? (
                      <>
                        <div className="absolute inset-0 z-10 flex items-start justify-center pt-3">
                          <Badge variant="destructive" className="bg-destructive text-destructive-foreground shadow-sm">
                        ...
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          className="absolute bottom-2 right-2 z-20 h-7 gap-1 px-2 text-xs bg-background shadow-sm hover:bg-accent hover:text-accent-foreground"
