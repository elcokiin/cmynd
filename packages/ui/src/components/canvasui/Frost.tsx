...
/**
 * Frosted-glass pane rendered as an OVERLAY on top of its children.
 *
 * The children stay in the normal document flow (their layout can never be
 * altered by the frost). A WebGL canvas is absolutely positioned on top and
 * paints a semi-transparent ice layer with `pointer-events: none` — hovering
 * melts a hole through the frost and reveals the real content underneath.
 *
 * This avoids the experimental html-in-canvas / drawElementImage path, which
 * lays content out inside a canvas and can break flex/grid layouts in
 * Chromium. With this approach the frost cannot break any layout, in any
 * browser.
 */
export function Frost({ children, className, style, ...options }: FrostProps) {
  ...
  return (
    <div className={className} style={{ position: "relative", ...style }}>
      {/* Real content — always in normal document flow, so the frost can never break it */}
      <div
        ref={contentRef}
        style={{
          position: "relative",
      ...
      {/* Dummy source canvas — kept for the vanilla engine API. Not used for capture. */}
      <canvas ref={sourceRef} aria-hidden style={{ display: "none" }} />
      {/* Frost overlay on top of the content */}
      <canvas
        ref={outputRef}
        aria-hidden
        style={{
